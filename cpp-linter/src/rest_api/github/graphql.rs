...
use crate::{
    ...
    rest_api::{send_api_request, RestApiClient, COMMENT_MARKER},
};
...
impl GithubApiClient {
    ...
    async fn get_existing_review_comments(&self, no_dismissed: bool) -> Result<Vec<ReviewThread>> {
        ...
        let (repo_owner, repo_name) = repo.split_once('/').ok_or(anyhow!(
            "Failed to split repository name into OWNER/NAME parts"
        ))?;
